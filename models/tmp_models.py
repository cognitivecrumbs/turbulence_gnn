...
class NodeModel(torch.nn.Module):
    ...
    def forward(self, x, edge_index, edge_attr, f=None, u=None, batch=None):
        ...
        if f is not None:
            out = torch.cat([x, out, f], dim=2)
        elif u is not None:
            out = torch.cat([x, out, u[batch]], dim=2)
        else:
...
class MeshGraphNetOld(torch.nn.Module):
    def __init__(self, args, dims):
        super(MeshGraphNetOld, self).__init__()
        # Arguments
        passes = args.passes
        n_hidden = args.n_hidden
        dim_hidden = args.dim_hidden
    ...
    def forward(self, z, edge_index, q_0): 
        '''Pre-process'''
        ...
        edge_attr = edge_attr.unsqueeze(0).repeat(x.shape[0],1,1)
